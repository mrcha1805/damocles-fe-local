...
export class ApiService {
  ...
  getProjectAPI(id: string): Observable<IProjectModel> {
    return this.http
      .get<IProjectModel>(
        this.endpoint + this.userProfile + '?ID=' + id,
        this.httpOption
      )
      .pipe(
        catchError((error: HttpErrorResponse) => {
  ...
  //Get ProjectID
  dynamicProjectIDMockup(
    name: string
  ): Observable<HttpResponse<IGetprojectID>> {
    return this.http.get<IGetprojectID>(
      'assets/data/get-project-id-mockup.json',
      {
        observe: 'response',
      }
    );
